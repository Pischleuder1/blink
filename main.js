...
class BlinkAdapter extends utils.Adapter {
	...
	const streamBatteryMinLevel = Math.max(0, Math.min(3, Number(this.config.streamBatteryMinLevel ?? 2)));
	...
	await this.ensureState(`${base}.status.smart_detection`, 'Smart Detection aktiv', 'boolean', 'indicator', false);
	...
	await this.ensureState(`${base}.status.smart_detection_raw`, 'Smart Detection Rohdaten', 'string', 'json', false);
	...
	for (let i = 0; i < 10; i++) {
		await this.ensureState(`${base}.video.history.${i}.file`, `History ${i} – MP4-Datei`, 'string', 'text', false);
		await this.ensureState(`${base}.video.history.${i}.timestamp`, `History ${i} – Zeitstempel`, 'string', 'date', false);
		await this.ensureState(`${base}.video.history.${i}.id`, `History ${i} – Clip-ID`, 'string', 'text', false);
		await this.ensureState(`${base}.video.history.${i}.source`, `History ${i} – Quelle (cloud|local_storage)`, 'string', 'text', false);
	}
	...
	await this.ensureState(`${base}.live.stream_active`, 'Stream wird aktuell gepollt', 'boolean', 'indicator', false);
	await this.ensureState(`${base}.commands.live_request`, 'Stream-Polling manuell aktivieren (60s)', 'boolean', 'button', true);
	...
	for (const [devId, cam] of this.camerasById.entries()) {
		...
		return await blinkApi.snapshotBuffer(
			this.session,
			cam.network_id,
			cam.id,
			cam.thumbnail,
	...
	try {
		...
		if (cmd === 'motion_detect') {
			...
			const res = await blinkApi.downloadLatestVideoSmart(
				this.session, cam.network_id, cam.id, cam.name, file,
	...
	try {
		...
		try {
			...
			res = await blinkApi.downloadLocalClip(
				this.session, cam.network_id,
				this.findSyncIdForNetwork(cam.network_id),
				localManifest.manifestId, localClip.id, file,
			);
	...
	const { clips: wanted, source } = await blinkApi.getHistoryClips(
		this.session, cam.network_id, cam.id, cam.name,
	...
	for (let i = 0; i < wanted.length; i++) {
		...
		if (source === 'cloud') {
			...
			this.session, cam.network_id, syncId,
			manifestId, clip.id, tmpFile(i),
			);
		}
		} catch (e) {
	...
	for (let i = wanted.length; i < HISTORY_SIZE; i++) {
		try {
			...
			// Slot-Datei existiert evtl. nicht – das ist OK.
		}
	}
	...
	await this.setStateAsync(`cameras.${devId}.status.smart_detection_raw`, String(summary?.smart_detection_raw || ''), true);
