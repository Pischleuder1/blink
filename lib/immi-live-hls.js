...
const FFMPEG_PATH = process.env.IMMI_FFMPEG_PATH || process.env.FFMPEG_PATH || '';
const DEBUG =
...
function startFfmpeg() {
	...
	log('Starte ffmpeg:', `${resolveFfmpegPath()} ${args.join(' ')}`);
	const ff = spawn(resolveFfmpegPath(), args, { stdio: ['pipe', 'ignore', 'pipe'] });
	ff.on('error', err => {
		if (err && err.code === 'ENOENT') {
			log(
				`FEHLER: ffmpeg nicht gefunden (versucht: '${resolveFfmpegPath()}'). ` +
					`Bitte ffmpeg installieren oder den vollen Pfad per Umgebungsvariable IMMI_FFMPEG_PATH setzen, z. B. IMMI_FFMPEG_PATH=/usr/local/bin/ffmpeg`,
			);
		} else {
			log('ffmpeg Spawn-Fehler:', err?.message || err);
		}
	});
...
/**
 * Ermittelt den ffmpeg-Pfad: zuerst die Umgebungsvariable IMMI_FFMPEG_PATH/FFMPEG_PATH,
 * dann gängige Installationsorte, sonst der bloße Name 'ffmpeg' (PATH-Auflösung).
 *
 * @returns {string} Pfad oder Name der ffmpeg-Binary.
 */
function resolveFfmpegPath() {
	if (FFMPEG_PATH) {
		return FFMPEG_PATH;
	}
	const candidates = ['/usr/bin/ffmpeg', '/usr/local/bin/ffmpeg', '/bin/ffmpeg', '/opt/ffmpeg/ffmpeg'];
